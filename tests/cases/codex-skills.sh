...
validate_charter_run_references() {
  local skill_dir=$1
  local required
  local example
  local specs_dir
  ...
  for example in index.json context.md spec.md checklists.md plan.md tasks.md; do
    if [[ ! -f "${skill_dir}/examples/${example}" ]]; then
      printf 'ERROR: charter-run missing examples/%s\n' "${example}" >&2
      failed=1
    fi
  done
  ...
  if [[ -f "${skill_dir}/examples/index.json" ]]; then
    if ! jq empty "${skill_dir}/examples/index.json" >/dev/null; then
      printf 'ERROR: charter-run examples/index.json is not valid JSON\n' >&2
      failed=1
    else
      ...
      if [[ -n "${specs_dir}" && -n "${active}" && "${specs_dir}" != "${active}" ]]; then
        printf 'ERROR: charter-run example context Specs Dir does not match index active\n' >&2
        failed=1
      fi
    fi
  fi
...
validate_skill() {
  ...
  validate_charter_run_references "${skill_dir}" || failed=1
