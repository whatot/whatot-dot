...
apply_apt_mirror() {
  if [[ -r /etc/os-release ]]; then
    ...
    echo "unsupported debian: missing /etc/os-release" >&2
    exit 1
  fi

  if [[ ${ID:-} != "debian" ]]; then
    echo "unsupported debian bootstrap target: ${ID:-unknown}" >&2
    exit 1
  fi

  local codename=${VERSION_CODENAME:-}
  if [[ -z "${codename}" ]]; then
    echo "unsupported debian: missing VERSION_CODENAME" >&2
    exit 1
  fi
